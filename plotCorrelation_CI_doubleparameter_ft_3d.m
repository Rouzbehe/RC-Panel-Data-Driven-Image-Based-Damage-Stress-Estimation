function [] = plotCorrelation_CI_doubleparameter_ft_3d(tvp,tvp2,plotfname,testfeatNo)
...
IA2 =1 - sum((tvp2(:,2)-tvp2(:,1)).^2)/sum((abs(tvp2(:,2)-mean(tvp2(:,1)))+abs(tvp2(:,1)-mean(tvp2(:,1)))).^2);
...
nameFolder='March_8_Monot_fiber_Spe_ft_indirect';
...
elseif Plot_No1==28
  ...
  ylabel('Predicted failure ratio (shear strain/ultimate shear strain)','fontsize',size);    
   ...
   elseif Plot_No1==29
  xlabel('True scaled shear stress   (\tau/f_t) ','fontsize',size);
  ylabel('Predicted scaled shear stress (\tau/f_t) ','fontsize',size);  

  NameJPG2 = strcat(nameFolder,'/',plotfname,'ConfidenceInterval');
  
     elseif Plot_No1==30
  xlabel('True maximum shear stress   (\tau_{max}) (MPa) ','fontsize',size);
  ylabel('Predicted maximum shear stress (\tau_{max}) (Mpa) ','fontsize',size);  
  NameJPG2 = strcat(nameFolder,'/',plotfname,'ConfidenceInterval');
  
     elseif Plot_No1==31
  xlabel('True principle stress   (\sigma_{1}) (MPa) ','fontsize',size);
  ylabel('Predicted principle stress (\sigma_{1}) (Mpa) ','fontsize',size);  
  NameJPG2 = strcat(nameFolder,'/',plotfname,'ConfidenceInterval');
  
     elseif Plot_No1==32
  xlabel('True principle stress   (|\sigma_{2}|) (MPa) ','fontsize',size);
  ylabel('Predicted principle stress (|\sigma_{2}|) (Mpa) ','fontsize',size);  
  NameJPG2 = strcat(nameFolder,'/',plotfname,'ConfidenceInterval');  
  
       elseif Plot_No1==33
  xlabel('True principle stress   (|\sigma_{x}|) (MPa) ','fontsize',size);
  ylabel('Predicted principle stress (|\sigma_{x}|) (Mpa) ','fontsize',size);  
  NameJPG2 = strcat(nameFolder,'/',plotfname,'ConfidenceInterval');  
  
      elseif Plot_No1==34
  xlabel('True scaled maximum shear stress   (\tau_{max}/f_t)  ','fontsize',size);
  ylabel('Predicted scaled maximum shear stress (\tau_{max}/f_t)  ','fontsize',size);  

  NameJPG2 = strcat(nameFolder,'/',plotfname,'ConfidenceInterval'); 
  
  
     elseif Plot_No1==35
  xlabel('True scaled principle stress   (\sigma_{1}/f_t) ','fontsize',size);
  ylabel('Predicted scaled principle stress (\sigma_{1}/f_t) ','fontsize',size);  
  NameJPG2 = strcat(nameFolder,'/',plotfname,'ConfidenceInterval');
  
     elseif Plot_No1==36
  xlabel('True scaled principle stress   (\sigma_{2}/f_t) ','fontsize',size);
  ylabel('Predicted scaled principle stress (\sigma_{2}/f_t) ','fontsize',size);  

  NameJPG2 = strcat(nameFolder,'/',plotfname,'ConfidenceInterval');
  
     elseif Plot_No1==37
  xlabel('True scaled stress (-\sigma_{x}/f_t) ','fontsize',size);
  ylabel('Predicted scaled stress (-\sigma_{x}/f_t) ','fontsize',size);  
%   xlabel('True scaled stress (|\sigma_{x}/\surd{f_t}|) ','fontsize',size);
%   ylabel('Predicted scaled stress (|\sigma_{x}/\surd{f_t}|) ','fontsize',size);    

  NameJPG2 = strcat(nameFolder,'/',plotfname,'ConfidenceInterval');  
  
     elseif Plot_No1==40
  xlabel('True principle stress theta (\theta_{p}) ','fontsize',size);
  ylabel('Predicted principle stress theta (\theta_{p}) ','fontsize',size);  
%   xlabel('True scaled stress (|\sigma_{x}/\surd{f_t}|) ','fontsize',size);
%   ylabel('Predicted scaled stress (|\sigma_{x}/\surd{f_t}|) ','fontsize',size);    
...
if Plot_No2==37
    ...
    titleString = 'Err |\sigma_{x}/f_t| estimation';
    set(colorTitleHandle ,'String',titleString,'fontsize',size);
    
elseif   Plot_No2==29
    
        titleString = 'Err |\tau/f_t| estimation';
    set(colorTitleHandle ,'String',titleString,'fontsize',size);
...
[n1,ctr1] = hist(tvp(:,1),20);
[n2,ctr2] = hist(tvp(:,2),20);

bar(ctr1,-n1,1);
axis([plotmin plotmax -max(n1)*1.1 0]);
axis('off');
h2 = gca;
subplot(2,2,1);
barh(ctr2,-n2,1);
axis([-max(n2)*1.1 0 plotmin plotmax]);
axis('off');
h3 = gca;
