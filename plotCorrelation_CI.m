...
IAold = 1 - mean((tvp(:,2)-tvp(:,1)).^2)/max(mean((abs(tvp(:,2)-mean(tvp(:,1)))+abs(tvp(:,1)-mean(tvp(:,1)))).^2),eps);
...
nameFolder='june_25_train_entirepanel';
...
elseif Plot_No==28
  ...
  ylabel('Predicted scaled maximum shear stress (\tau_{max}/\surd{f_t})  ','fontsize',size);  

  NameJPG2 = strcat(nameFolder,'/',plotfname,'ConfidenceInterval'); 
  
  
     elseif Plot_No==35
  xlabel('True scaled principle stress   (\sigma_{1}/\surd{f_t}) ','fontsize',size);
  ylabel('Predicted scaled principle stress (\sigma_{1}/\surd{f_t}) ','fontsize',size);  
  NameJPG2 = strcat(nameFolder,'/',plotfname,'ConfidenceInterval');
  
     elseif Plot_No==36
%   xlabel('True scaled principle stress   (\sigma_{2}/\surd{f_t}) ','fontsize',size);
%   ylabel('Predicted scaled principle stress (\sigma_{2}/\surd{f_t}) ','fontsize',size);  
  ...
  ylabel('Calculated principle stress theta (\theta_{p}) ','fontsize',size);  
%   xlabel('True scaled stress (|\sigma_{x}/\surd{f_t}|) ','fontsize',size);
%   ylabel('Predicted scaled stress (|\sigma_{x}/\surd{f_t}|) ','fontsize',size);    
